...
test.describe('Agent Detail Page', () => {
  test('should load hephaestus agent detail page', async ({ page }) => {
    await page.goto('/agents/hephaestus')
    await expect(page.locator('h1')).toContainText('Hephaestus')
  })

  test('should display agent description', async ({ page }) => {
    await page.goto('/agents/hephaestus')
    // Hephaestus description includes "autonomous"
  ...
  test('should show breadcrumb navigation', async ({ page }) => {
    ...
    await expect(breadcrumbNav.locator('text=Hephaestus')).toBeVisible()
  })
